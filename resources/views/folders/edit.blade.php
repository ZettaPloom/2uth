...
@section('content')
    <section class="max-w-min p-6 mx-auto bg-white rounded-md shadow-md dark:bg-gray-800">
        <h2 class="text-lg font-semibold text-gray-700 capitalize dark:text-white">Change the folder name</h2>

        <form action="{{ route('folders.update', $folder->id) }}" method="post">
            @csrf
            @method('PATCH')
            <div>
                <label class="text-gray-700 dark:text-gray-200" for="username">Folder name</label>
